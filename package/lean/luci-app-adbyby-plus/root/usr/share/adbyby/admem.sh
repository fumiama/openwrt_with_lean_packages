...
if [ ! -f "/tmp/adbyby.updated" ];then
  ...
  while [ "$wget_ok" = "0" ] 
    do 
    wget --spider --timeout=3 http://www.baidu.com
    if [ "$?" == "0" ]; then
      wget_ok="1"
      
      touch /tmp/md5.json && wget --no-check-certificate -T 10 -O /tmp/md5.json https://adbyby.coding.net/p/xwhyc-rules/d/xwhyc-rules/git/raw/master/md5.json
      ...
      if [ "$adm5" == "$bmd5" ];then
        ...
        touch /tmp/lazy.txt && wget --no-check-certificate -T 10 -O /tmp/lazy.txt https://adbyby.coding.net/p/xwhyc-rules/d/xwhyc-rules/git/raw/master/lazy.txt
        touch /tmp/video.txt && wget --no-check-certificate -T 10 -O /tmp/video.txt https://adbyby.coding.net/p/xwhyc-rules/d/xwhyc-rules/git/raw/master/video.txt
